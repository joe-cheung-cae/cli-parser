...
struct Argument : ArgumentBase {
    ...
    void set_value(const std::string& str) override { value = TypeConverter<T>::convert(str); parsed = true; }
};
...
public:
    ...
    void add_option(const std::string& short_name, const std::string& long_name, const std::string& description, bool required = false, T* default_value = nullptr) {
        auto arg = std::unique_ptr<Argument<T>>(new Argument<T>());
            ...
            if (arg.size() > 1 && arg[0] == '-') {
                ...
                if (it != name_to_arg_.end()) {
                    if (++i < argc) {
                        ...
                        return ParseResult{false, "Missing value for " + name};
                    }
                } else {
                    return ParseResult{false, "Unknown option: " + arg};
                }
            }
            ++i;
        }
